...
extractor.prototype.extract_data = function(url, content, extract_rule, uppper_data, dom) {
        ...
        if (this.spiderCore.settings.extractorType && this.spiderCore.settings.extractorType === 'column') {
            var baser = content;
            var ruleUnique = null;
            Object.keys(extract_rule['rule']).map(ruleItem => {
                if (extract_rule['rule'][ruleItem].unique) ruleUnique = extract_rule['rule'][ruleItem];
            })
            if (ruleUnique['base'] === 'url') baser = url;
            switch (ruleUnique['mode']) {
                case 'regex':
                    var tmp_result = this.regexSelector(baser, ruleUnique['expression'], ruleUnique['index']);
                    data['column'] = tmp_result;
                    break;
                case 'xpath':
                    break;
                case 'value':
                    data['column'] = ruleUnique['expression'];
                    break;
                case 'json':
                    break;
                default: //css selector
                    ...
                    var pick = ruleUnique['pick'];
                    if (ruleUnique['subset']) {
                        pick = false;
                        (function(k) {
                            var result_arr = [];
                            var tmp_result = self.columnSelector(baser, extract_rule['rule'], ruleUnique['expression'], pick, ruleUnique['index']);
                            if (tmp_result) {
                                tmp_result.each(function(x, elem) {
                                    var sub_dom = tmp_result.eq(x);
                                    result_arr.push(self.extract_data(url, content, ruleUnique['subset'], data, sub_dom));
                                });
                            }
                        ...
                        try {
                            var tmp_result = self.columnSelector(baser, extract_rule['rule'], ruleUnique['expression'], pick, ruleUnique['index']);
                            if (tmp_result && !isEmpty(tmp_result)) data['column'] = tmp_result;
                        } catch (e) {
 ...
 * @returns {*}
 */
extractor.prototype.columnSelector = function($, extract_rule, expressionUnique, pick, index) {
    // if (!index) index = 1;
    // var real_index = parseInt(index) - 1;
    var tmp_val = $.find(expressionUnique);
    if (!pick) return tmp_val;
    if (typeof(tmp_val) === 'object') {
        // if (real_index >= 0) {
        // var val = tmp_val.eq(real_index);
